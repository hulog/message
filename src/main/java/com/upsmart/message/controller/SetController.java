...
import com.upsmart.message.converter.EinterfaceConverter;
...
public class SetController {
    ...
    @Autowired
    private EinterfaceConverter einterfaceConverter;
    ...
    public BaseMessage setemail(@RequestParam(value = "title", required = true) String title,
        ...
        try {
            if (null == this.setService.setemail(title, host, port, ename, epassword)) {
                ...
                ResponseUtil.buildResMsg(message, StatusCode.ERROR,
                        this.setService.setemail(title, host, port, ename, epassword));
            }
        } catch (Exception e) {
    ...
    @ResponseBody
    public BaseMessage findConfig(@RequestParam(value = "inftype") Integer inftype) {
        BaseMessage message = new BaseMessage();
        if (null != inftype) {
            EinterfaceDto eDto = this.einterfaceConverter.toDto(this.setService.findConfig(inftype));
            if (null != eDto) {
                message.setData(eDto);
                ResponseUtil.buildResMsg(message, StatusCode.SUCCESS);
            } else {
                ResponseUtil.buildResMsg(message, StatusCode.ERROR);
            }
